...
type DisplayColumnsInterface = keyof UserInterface | 'action';
@Component({
    selector: 'app-user',
    templateUrl: '../page/user.page.html',
    styleUrls: ['../style/user.style.scss'],
})
export class UserComponent implements AfterViewInit {
    displayedColumns: DisplayColumnsInterface[] = ['name', 'condition', 'action'];
    ...
    onAdd(){
        this.router.navigate(['/app/user/new']);
    }

    onEdit(user: UserInterface) {
        this.router.navigate(['/app/user/'+user.uuid]);
    }
    ...
    onLoadUsers(filter: Partial<UserInterface>) {
        return this.userService.onFind(filter).subscribe((result) => {
            if (result?.statusCode && result?.statusCode != 200) {
                this.matSnackBar.open(result?.message ?? 'Ocurrio un error al filtrar el servidor.');
                return;
            }
            const newData: UserInterface[] = [...this.dataSource?.data ?? [], ...result];
        ...
        if (this.dataSource) {
            ...
            if (this.dataSource.filteredData.length == 0)
                for (let index = 0; index < this.displayedColumns.length; index++) {
                    ...
                    const payload: Partial<UserInterface> = { pagination: { offset: 0, limit: 5 } };
                    if (value == 'name') payload.name = filter;
                    if (value == 'condition') payload.condition = Boolean(filter);

                    if (value != 'action') {
                        await (new Promise((resolve) => {
                            this.onLoadUsers(payload).add(() => {
                                this.dataSource.filter = filter;
                                resolve(null);
                            });
                        }))
